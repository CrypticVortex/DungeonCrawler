...
public class CommandTutorial implements CommandExecutor {
	public String[][] tutorials = {
		...
		{ "To select a class type \"/class\", or right click the class selection sign in the lobby. You're allowed to change your class at any point outside of a dungeon if you want to try something new. Each group may only contain one of each class!" }, // Classes segment
		...
		{ "Talents are passive traits that will boost various stats. These will allow you to specialize in doing more damage, doing more healing, or even taking less damage, allowing you to play how you prefer." }, // Talents segment
		{ "Pre-built dungeons will contain many secrets and objective-based segments! For instance killing x amount of mobs, or hitting x block will do something on the map. Pay close attention to the map layout!" } // Dungeons segment
	};
...
//		DPlayer pl = DungeonCrawler.players.get(player.getUniqueId());
		if(args.length == 0) {
			player.sendMessage("§6There are (" + tutorials.length + ") different categories of tutorial messages; §aabout, classes/class, experience/exp, loot, talents, and dungeons. §6To view one simply type §c/tutorial <category>");
		}
		if(args.length == 1) {
			...
			if(args[0].equalsIgnoreCase("dungeons")) player.sendMessage("§b" + tutorials[5][0]);			
		}
		return true;
	}
